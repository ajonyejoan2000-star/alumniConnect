...
function openModal() {
    ...
    async function handleSubmit(e) {
      ...
      try {
        ...
        // Uncomment and adjust when wiring up:
        //
...
// ============================================================
    ...
    if (!state.opportunities.length) {
      container.innerHTML = `<div class="empty-state">
        ...
        <p>${state.activeTab === 'saved' ? 'You haven\'t saved any opportunities yet.' : 'Try adjusting your filters or check back later.'}</p>
      </div>`;
      return;
    }
  ...
  function updateResultsCount() {
    ...
    document.getElementById('resultsCount').innerHTML = `Showing <strong>${state.opportunities.length}</strong> of <strong>${state.total}</strong> ${label}`;
  }
  ...
  function openPostModal() {
    if (!getToken()) { showToast('Please sign in to post an opportunity', 'error'); return; }
  ...
  async function submitOpportunity() {
    ...
    if (!title || !type || !description) {
      showToast('Title, type, and description are required', 'error'); return;
    }
    ...
    try {
      ...
      btn.textContent = 'Post Opportunity'; btn.disabled = false;
    }
  }

  function clearForm() {
    ['f_title','f_type','f_description','f_company','f_location','f_salary','f_deadline','f_skills','f_link','f_category'].forEach(id => {
      document.getElementById(id).value = '';
    });
    document.getElementById('f_remote').checked = false;
  }
  ...
  async function viewOpportunity(id) {
    ...
    document.getElementById('viewModalBody').innerHTML = `<div style="text-align:center;padding:3rem;color:var(--text-muted)">Loading...</div>`;
    ...
    try {
      ...
      document.getElementById('viewModalBody').innerHTML = `<p style="color:var(--red);text-align:center;padding:2rem">${err.message}</p>`;
    }
  }
  ...
  function renderViewBody(opp) {
    ...
    const alreadyApplied = token && opp.applicants?.some(a => a.user?._id === state.currentUser || a.user === state.currentUser);
    ...
    let html = `
      <div class="view-header-banner">
        ...
        <div class="view-meta-row">
          ...
          ${opp.applicants?.length != null ? `<span class="meta-item">${iconUser()} ${opp.applicants.length} applicant${opp.applicants.length !== 1 ? 's' : ''}</span>` : ''}
        </div>
      </div>`;
    ...
    if (poster.name || poster.email) {
      html += `<div class="view-section-title">Posted by</div>
               <div class="poster-row">
                 <div class="poster-avatar">${poster.profilePicture ? `<img src="${poster.profilePicture}" alt="${poster.name}">` : avatarText(poster.name || 'U')}</div>
    ...
    if (!expired && !alreadyApplied && !isOwner) {
      html += `<button class="btn btn-primary" onclick="applyToOpportunity('${opp._id}', this)">${alreadyApplied ? '✓ Applied' : '🚀 Apply Now'}</button>`;
      ...
      opp.applicants.forEach(a => {
        ...
        html += `<div class="applicant-row">
          <div style="display:flex;align-items:center;gap:0.6rem">
            <div class="poster-avatar" style="width:30px;height:30px;font-size:0.75rem">${u.profilePicture ? `<img src="${u.profilePicture}">` : avatarText(u.name || 'U')}</div>
            <div><strong style="font-size:0.875rem">${escHtml(u.name || 'Applicant')}</strong>
  ...
  async function applyToOpportunity(id, btn) {
    ...
    try {
      ...
      btn.textContent = '✓ Applied'; btn.classList.remove('btn-primary'); btn.classList.add('btn-ghost');
      // Refresh card
  ...
  async function toggleSave(id, btn) {
    ...
    try {
      ...
      // Check if saved array includes current user
      const opp = data.data;
      const userId = state.currentUser;
      const nowSaved = opp?.saved?.includes(userId) ?? !state.savedIds.has(id);

      if (nowSaved) { state.savedIds.add(id); btn.classList.add('saved'); showToast('Saved ⭐', 'success'); }
      else { state.savedIds.delete(id); btn.classList.remove('saved'); showToast('Removed from saved', 'info'); }
  ...
  async function toggleSaveFromModal(id, btn) {
    ...
    try {
      ...
      if (nowSaved) { state.savedIds.add(id); btn.textContent = '★ Unsave'; btn.className = 'btn btn-danger'; showToast('Saved ⭐', 'success'); }
      else { state.savedIds.delete(id); btn.textContent = '☆ Save'; btn.className = 'btn btn-ghost'; showToast('Removed from saved', 'info'); }
  ...
  function iconCal() { return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>`; }
  ...
  function initUser() {
    ...
    try {
      ...
      if (payload.name) {
        ...
        el.textContent = `Hi, ${payload.name.split(' ')[0]}`;
...
//   // ============================================================
  async function fetchOpportunities(page = 1) {
    ...
    const { search, type, location, skill, remote } = state.filters;
      ...
      <div class="opp-card" onclick="viewOpportunity('${opp._id}')" style="animation-delay:${idx * 0.06}s; ${expired ? 'opacity:0.6' : ''}">
        <div class="card-top">
          <div class="card-avatar">
            ${poster.profilePicture ? `<img src="${poster.profilePicture}" alt="${poster.name}" onerror="this.style.display='none'">` : initials}
          </div>
        ...
        ${skills.length ? `<div class="card-skills">${skills.map(s => `<span class="skill-tag">${escHtml(s)}</span>`).join('')}${(opp.skills||[]).length > 3 ? `<span class="skill-tag">+${opp.skills.length - 3}</span>` : ''}</div>` : ''}
